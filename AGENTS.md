...
- `scripts/` —— **运维/离线脚本**（不在运行时链路上）：`ledger-audit.mjs`（只读账本体检，含画像段/候选池/冷存）、`ledger-quarantine-candidates.mjs` + `ledger-quarantine-apply.mjs`（存量隔离候选与执行器，默认 dry-run）、`dream.mjs`（离线巩固运行器）、`dream-review.mjs`（候选池人工审）、`dream-export.mjs`（候选 → weaver JSONL）、`build-client.mjs`
