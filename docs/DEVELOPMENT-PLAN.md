...
| ACP-B14 | 计划 | 规则层容量治理（B10 复查剩余项）：① 候选 `slice(0,3)` 时间序 → 应改显式优先级（否则老规则永不进候选）② 规则行 source 标签固定开销 20 token/条（`[acp:rule \| id=… \| domain=…]`）→ 瘦身为短标签可多容纳 2-3 条 ③ 规则创建时校验长度上限（>40 字提示不可注入） | 设计决策：优先级字段 / 域权重 / 全量按需；标签瘦身涉及渲染格式（golden 测试同步） | planned |
