...
→ **三机实复测（2026-09-22 22:1x）**：A / B / W **各自 11 个库全部 `wal`**，`-wal` 文件全为 0 字节（已 checkpoint）。
**结论：「weaver 库不是 WAL」在本轮三机上都不成立，W 也不是例外** —— 本节早先那句「W 全部是 delete」来自一次**未独立复核的转述**，已本人复测推翻（教训：第三方结论进设计文档前先自己跑一遍）。
因此 §10.3 的「直写被并发挡住」这条风险在三机上都不成立。
...
## 13. weaver 侧蒸馏的执行实况（2026-09-23，对 dreaming 有直接影响的部分）

> 蒸馏 weaver 与 dreaming 是两条管线（见 `docs/plans/weaver-distillation-framing-20260922.md` §1）。这里只记**会影响 dreaming 设计的那几条实测**。

### 13.1 读侧已经有闸门了（2026-09-23 生效）

`weaver_read` 默认只给 `metadata + summary + 正文前 1200 字`，`full=true` 才给全文。实测最长条目（49,828 字 ≈ 24,914 tokens）**50,127 → 2,056 字符（24.4×）**。

含义：**dreaming 产出的候选如果最终落进 weaver，被读到的路径已经变了**——读侧不再无脑拉全文，"把长文写进 weaver" 的代价模型与半年前不同了。

### 13.2 零访问不再是可靠判据（先冻结再判）

`access_count / last_access` **每次检索都会被改写**（`plugins/dsh-weaver/lib/weaver-query.mjs` 的 `bumpAccess`）。所以"这条零访问"必须在**带时间戳的快照**上判，不能在活库上判。

- 工具：`.tooling/scripts/weaver-access-snapshot.mjs`（只读；`--out` 冻结 / `--diff` 比对）
- 基线：9 库 / 22,229 条 / 零访问 21,602（**97.2%**）
- ⚠️ **零访问 ≠ 无价值**：命中测试显示，问对主题仍能捞出来（只是从未被"读进去"）。**不要用零访问做删除依据**（死螺旋风险）。

### 13.3 三条给任何"自动写 weaver"的实现的硬约束

| 约束 | 依据 |
|---|---|
| **tags 必须写成 JSON 数组**（`["a","b"]`），不能写 `a,b` 逗号串 | 实测库里 124 条历史条目是逗号串；`wv patch --add-tags` 遇到它们会 `JSON.parse` 失败→兜成空→**把原有标签整条冲掉** |
| **内容字段不能以 `--` 开头** | `wv.mjs` 的 `opts()` 把 `--xxx` 当 flag；`--summary` 值是 `---` 开头的条目实测报 `cannot be bound to SQLite parameter 1`。含 `--` 开头的正文/摘要要加前导空格或走文件 |
| **删除走 `wv rm`（记墓碑），不要直连 sqlite** | 直写会跳过 `ensureDailyBackup` / `notePending`，且不更新 `entry_state.pushed_hash` → `wv-merge` 会拿旧 baseHash 判断，**可能把主库静默覆盖成陈旧值**（`wv.mjs` 内记着 2026-09-15 实测复现） |

### 13.4 归并的方法学（若 dreaming 将来也要做合并）

**"正文取并集"是错的**：对"同一素材的两种改写"，段落级比对会判被并方 93% 都是"独有" → 实测膨胀 **1.68×**（等于把同一件事用两套措辞各存一份）。

**正确做法**：留一条，正文取**结构更完整**的那份（结构分 = 标题数 + 2×表格行数；打平取更新时间更近者），其余字段取并集（tags 并、source 追加、confidence 取高）。落地结果：**正文总长零膨胀**。

### 13.5 第二次实测：可导出的 5 条里 4 条是「进度快照」（2026-09-24）

§13.1 的定性判断（复现能选出「值得一提的」，但不是「可用的」）今天拿到了更具体的数字。
对 A 机账本跑 `dream-export.mjs --state consensus`：

| 项 | 数 |
|---|---|
| consensus 候选 | **51** |
| 落在导出白名单（work / external_fact） | **5** |
| 被挡下 | 46（其中绝大多数是 `experience`） |

**逐条看那 5 条**：4 条是「进度快照 / 状态通报」，只有 1 条是可复用知识。

- ✗ 「**当前在** dsh-desktop-shell 的某分支上先做提交；需排查右上角 unavailable…」
- ✗ 「W机已启动且未安装过 DSH」
- ✗ 「**已读完**三份报告…**正复现** git-guardrails 串行描述 bug」
- ✗ 「在 …apps\desktop 下执行 pnpm tauri build，退出码 0」（一次性操作记录）
- ✅ 「Plugin bundle structure is package.json + cordis.patch.yml；对比了官方 master 与本地 alpha.1」

**根因**：Dreaming 按**复现次数**聚合，而「当前在做 X」这类进度快照天然会反复出现 ——
每次会话压缩、每个新会话的交接摘要都会重提一次，于是复现次数很高、内容却是一次性的。
**这与「会话→skills」那条线被否掉的原因是同一个**：`复现次数 ≠ 价值`，
而决定价值的是「产出物属于哪一类」（工程判断 vs 操作记录 vs 状态快照）。

**可操作的建议**（未实施，留给判据改造）：
1. 白名单之外再加一道**时态过滤**（含「当前/已/正…」等进度标记的候选降权或挡下）；
2. 或者把「进度快照」这类候选的**归宿改成 WorkState**（它本来就是干这个的），而不是 weaver；
3. 无论哪条，**不要在判据改好前批量 import** —— 污染检索的代价比不导高。
